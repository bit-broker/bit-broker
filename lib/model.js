...
class Connector {
    ...
    list() {
        return this._rows.orderBy('connector.name');
    }
    ...
    insert(name, details) {
        ...
        details.name = name;
        details.contribution_id = uuidv4();

        let insert = ['entity_id', 'name', 'description', 'webhook', 'cache', 'contribution_id'];
        let select = db.raw(`entity.id AS entity_id, :name AS name, :description AS description, :webhook AS webhook, :cache AS cache, :contribution_id AS contribution_id`, details);
        let insertion = db.raw('connector (??, ??, ??, ??, ??, ??)', insert);
        let selection = db.from('entity').where({ 'entity.name': this.entity_name }).select(select);

        return this._write.from(insertion).insert(selection).then (result => result.rowCount !== 0);
    }
    ...
    update(name, details) {
        if (details.webhook.length === 0) details.webhook = null;
        return this._write.where({ name }).update(details).then (result => result.rowCount !== 0);
    }

    // --- deletes a connector on the instance entity type

    delete(name) {
        return this._write.where({ name }).delete().then (result => result.rowCount !== 0);
    }
}
...
class Entity {
    ...
    list() {
        return this._rows.orderBy('entity.name');
    }
    ...
    insert(name, details) {
        details.name = name
        return this._rows.insert(details).then (result => result.rowCount !== 0);;
    }
